...
// Auto-update QR preview on settings change
[qrSizeSelect, qrDarkColorInput, qrLightColorInput, errorCorrectionSelect]
...
function setLanguage(lang) {
  ...
  document.title = translations[lang].title;
  contentInput.placeholder = translations[lang].pastePlaceholder[contentTypeSelect.value];
  localStorage.setItem('lang', lang);
}
...
function setTheme(theme, regenerate = true) {
  ...
  if (regenerate && qrCodeInstance) generateQrCode();
}
...
function updatePlaceholder() {
  ...
  contactFormContainer.style.display = (type === 'contact') ? 'block' : 'none';
  fileUploadContainer.style.display = (type === 'file') ? 'block' : 'none';
...
function generateQrCode() {
  ...
  if (type === "file") {
    ...
    if (!content) return alert(translations[languageSelect.value].alertEnter);
    if (type === 'url' && !/^https?:\/\//i.test(content)) content = 'https://' + content;
    if (type === 'phone') content = `tel:${content.replace(/\s/g, '')}`;
    if (type === 'sms') {
      const [num, msg] = content.split('|', 2);
      content = msg ? `SMSTO:${num}:${msg}` : `SMSTO:${num}`;
    }
    if (type === 'email') {
      const [addr, subject, body] = content.split('|');
      let mailto = `mailto:${addr}`;
  ...
  qrPlaceholder.style.display = 'none';
  resultContent.textContent = content;
  downloadLink.style.display = copyBtn.style.display = 'inline-flex';
  shareBtn.style.display = navigator.share ? 'inline-flex' : 'none';
...
function buildVCard() {
  const fields = {
    FN: document.getElementById('contactName').value.trim(),
    ...
    ADR: document.getElementById('contactAddress').value.trim(),
    URL: document.getElementById('contactWebsite').value.trim(),
    NOTE: document.getElementById('contactNote').value.trim()
  };
  let vcard = "BEGIN:VCARD\nVERSION:3.0\n";
  for (const [k, v] of Object.entries(fields)) if (v) vcard += `${k}:${v}\n`;
...
function resetUI() {
  ...
  [downloadLink, copyBtn, shareBtn, clearBtn].forEach(el => el.style.display = 'none');
  updatePlaceholder();
}

function copyContent() {
  navigator.clipboard.writeText(resultContent.textContent)
    .then(() => alert(translations[languageSelect.value].alertCopy))
